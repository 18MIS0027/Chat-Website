...
{
$sql=mysqli_query($con,"SELECT password FROM admin where password='".$_POST['password1']."' and id='".$_SESSION['aid']."'");
$num=mysqli_fetch_array($sql);
if($num>0)
{
 $con=mysqli_query($con,"update admin set password='".$_POST['password2']."' where id='".$_SESSION['aid']."'");
echo "<script>alert('Password changed successfully');</script>";
echo'<script> window.location="admin-logout.php"; </script> ';   
}
else
{
...
$stmt = $con->prepare('SELECT name,phoneno,email FROM admin WHERE id = ?');
